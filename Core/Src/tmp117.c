...
//	HAL_I2C_Master_Receive(&i2c,TMP117_DeviceID,buf,1,100);
	...
	while(1){
		HAL_I2C_Mem_Read(&i2c, TMP117_DeviceID, TMP117_ConfigurationRegister, I2C_MEMADD_SIZE_8BIT, buf, 2, 1000);
		uint16_t config = (buf[0]<<8)|buf[1];
		if (!((config>>13)&0x01)) break;
	}

	HAL_I2C_Mem_Read(&i2c, TMP117_DeviceID, TMP117_TemperatureRegister, I2C_MEMADD_SIZE_8BIT, buf, 2, 1000);
...
//	return ((buf[0]<<8)|buf[1]);
	...
	HAL_I2C_Mem_Read(&i2c, TMP117_DeviceID, TMP117_ID_Register, I2C_MEMADD_SIZE_8BIT, data, 2, 1000);
 ...
 *
 */
uint8_t Tmp117_Init(I2C_HandleTypeDef i2c){
	uint16_t device_id = TMP117_get_ID_Register(i2c);
	if (device_id != TMP117_Device_ID) {printf("TMP117 not found, %i\r\n",device_id); return 0;}
	else printf("TMP117 detected\r\n");
